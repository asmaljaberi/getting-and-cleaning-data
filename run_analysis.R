...
log <- function(...){ cat("[run_analysis.R] ", ..., "\n", sep="")}

codebook <- function(...){ cat(..., "\n", file = targetCodebookFilePath, append = TRUE, sep = "")}
...
if(debug && exists("mergedData")){ rm(mergedData)}
...
#codebook
targetCodebookFilePath <- "./data/CodeBook.md"
file.remove(targetCodebookFilePath)
codebook("# Code Book")
codebook("generated ", as.character(Sys.time())," during sourcing of `run_analysis.R`")
codebook("")

if(!exists("keyColumns")){ keyColumns <<- c() }

if(!exists("featureColumns")){ featureColumns <<- c() }
...
log("[#4] Appropriatelly labels the data set with descriptive variable names. ")
...
names(variableData) <- c("dimension", "source", "type", "jerk", "magnitude", "method", "exis")
